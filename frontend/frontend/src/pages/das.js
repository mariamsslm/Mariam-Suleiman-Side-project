...
const MemeDashboard = () => {
  ...
  const handleRemoveMeme = async (i) => {
    try {
      const response = await axios.delete(`${process.env.REACT_APP_PATH}/meme/delete/${i}`)
      console.log("Deleted article", response)
      const remainedArticles = data.filter((meme) => meme.id !== i)
  ...
  return (
    ...
    <Grid item xs={12} sm={8} md={6} lg={4}>
      <Paper elevation={3} style={{ padding: 20, backgroundColor: "#018CDD" }}>
  ...
  <Table sx={{}}>
    ...
    <TableCell>
      <img src={meme.imageURL} alt="Meme" style={{ width: '50px', height: '50px' }} />
    </TableCell>
    ...
    <TableCell>
      ...
      <Button
        variant="outlined"
        color="primary"
        sx={{ backgroundColor: "#F5D431", width: '10px',marginBottom:"5px" }}
      >
        <EditIcon/>
      </Button>
      </Link>
